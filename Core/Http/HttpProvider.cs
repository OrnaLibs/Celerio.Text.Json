...
public class DefaultHttpProvider : IHttpProvider
{
    ...
    public bool ParseRequest(NetworkStream stream, out HttpRequest request, out IHttpProvider.HttpParsingError error)
    {
        ...
        try
        {
            ...
            var transferEncoding = req.Headers.GetFirst("Transfer-Encoding");
            ...
            else if (req.Headers.TryGet("Content-Length", out string? contentLengthValue))
            {
                ...
                if (remaining > 0 && !sb.ReadExact(body, alreadyRead, remaining))
                {
                    error = IHttpProvider.HttpParsingError.IncompleteRequest;
                    return false;
                }
    ...
    private static bool ParseChunkedBody(StreamBuffer sb, MemoryStream decoded, out IHttpProvider.HttpParsingError error)
    {
        error = IHttpProvider.HttpParsingError.None;
        while (true)
        {
            ...
            var chunkSizeHex = (semicolonIndex >= 0 ? line.Substring(0, semicolonIndex) : line).Trim();
